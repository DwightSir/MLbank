...
lnmpfile='link-lnmp.zip';
webfile='link-web.zip';
backups='backups.zip';
phpmyadmin=sql$RANDOM$RANDOM;
llwswebfile='llws-web.zip';
uploadfile=link-openvpn.tar.gz;
export uploadfile=$uploadfile
clear
echo -e "\033[34m $wzlinkLogo \033[0m"
echo -n -e "请输入wzlink作者QQ： [\033[32m $key \033[0m] ："
read PASSWD
readkey=$PASSWD
echo 
...
echo
echo -e "\033[1;36m> 3 - 备份恢复 >> 备份和恢复wzlink 证书和数据库、以便重新搭建\033[0m"
echo
...
echo "对接失败，您的服务器还未搭建wzlink openvpn"
exit 0;
fi
clear
...
appopen=yes
echo
echo -n " 请输入流量卫士Key 【回车默认；wzlink666】："
read adminKey
if [[ -z $adminKey ]] 
then 
echo
echo -e "\033[34m已写入流量卫士对接KEY：wzlink666 \033[0m" 
adminKey=wzlink666
else 
echo
...
if [ ! -f "/root/beifen/open.sql" ]; then
	echo -e "  \033[31mwzlink 自带APP线路备份失败\033[0m"
else
	echo -e "  \033[32mwzlink 自带APP线路备份成功\033[0m"
fi
sleep 1
echo
...
then
			clear
			echo
			echo -e "\033[31m 请设置免流端口：（wzlink 自带440与3389端口请勿使用这两个端口）\033[0m"
			echo 
			...
			then 
			    ...
			    if [[ "$llwsinstall" == "1" ]];then
 			 	...
 			 	if [[ -z $llwsmm ]]
 			 	then
 			 	llwsmm=wz$RANDOM$RANDOM
 			 	fi
 			 		echo -e "\033[34m已设置流量卫士面板管理员密码为：$llwsmm \033[0m"
 			 		echo
 			 		echo -n -e "设置流量卫士APP名称【回车默认；wzlink 流量卫士】："
 			 		read llwsname
 			 	if [[ -z $llwsname  ]] 
 			 	then 
                llwsname=wzlink流量卫士
...
echo
echo
echo '欢迎使用wzlink OpenVPN快速安装脚本' >>info.txt
echo "
---------------------------------------------------------
